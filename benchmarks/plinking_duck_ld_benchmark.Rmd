...
Note: the fork now uses native PVAR region pushdown for `plink_ld()` when a `region` is supplied. On the real all-hg38 resource this cuts peak RSS substantially by materializing only the target interval's metadata; elapsed time is now dominated by the LD kernel / pgenlib reader state rather than all-PVAR string materialization.
